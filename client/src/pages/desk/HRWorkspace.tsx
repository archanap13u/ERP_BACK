...
import { Users, UserCheck, CalendarDays, Megaphone, TrendingUp, Plus, Clock } from 'lucide-react';
    ...
    const handleSaveFeatures = async (newFeatures: string[]) => {
        try {
            ...
            if (res.ok) {
                ...
                // Trigger sidebar reload if needed, or simply reload page
    ...
    return (
        <div className="space-y-8 pb-20">
            <Workspace
                ...
                summaryItems={[
                    { label: 'Total Employees', value: loading ? '...' : counts.employee || 0, color: 'text-blue-600', doctype: 'employee' },
                    { label: 'Today Present', value: loading ? '...' : counts.attendance || 0, color: 'text-emerald-600', doctype: 'attendance' },
                    { label: 'Pending Reviews', value: loading ? '...' : '2', color: 'text-orange-600', doctype: 'performancereview' },
                ]}
                masterCards={[
                    { label: 'Employees', icon: Users, count: counts.employee || 0, href: '/employee' },
                    { label: 'Attendance', icon: UserCheck, count: counts.attendance || 0, href: '/attendance' },
                    { label: 'Performance', icon: TrendingUp, count: '0', href: '/performancereview' },
                    { label: 'Holidays', icon: CalendarDays, count: counts.holiday || 0, href: '/holiday' },
                    { label: 'Announcements', icon: Megaphone, count: counts.announcement || 0, href: '/announcement' },
                ]}
                shortcuts={[
            ...
            <div className="max-w-6xl mx-auto pb-12">
                <div className="bg-gradient-to-br from-indigo-900 to-indigo-800 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="p-6 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm text-center transform hover:scale-105 transition-transform cursor-pointer">
                                <p className="text-4xl font-black text-white mb-1">{counts.employee || 0}</p>
                                <p className="text-[12px] text-indigo-200 font-bold uppercase tracking-widest">Active Staff</p>
                            </div>
                            <div className="p-6 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm text-center transform hover:scale-105 transition-transform cursor-pointer">
                                <p className="text-4xl font-black text-emerald-400 mb-1">{counts.attendance || 0}</p>
                                <p className="text-[12px] text-indigo-200 font-bold uppercase tracking-widest">Today's Presence</p>
                            </div>
                            <div className="p-6 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm text-center transform hover:scale-105 transition-transform cursor-pointer">
                                <p className="text-4xl font-black text-orange-400 mb-1">{counts.holiday || 0}</p>
                                <p className="text-[12px] text-indigo-200 font-bold uppercase tracking-widest">Annual Holidays</p>
                            </div>
                        </div>
                    </div>
                </div>
